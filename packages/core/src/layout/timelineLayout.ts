...
import type { LayoutTopic, TimelineBranchSide } from './layoutTypes.js';
import { visibleSubtopics } from '../model/topicTraversal.js';

export type TimelineLayoutMode = 'timeline' | 'timeline-up' | 'timeline-down';

export function layoutTimeline(
  root: LayoutTopic,
  collapsedIds: ReadonlySet<string>,
  mode: TimelineLayoutMode = 'timeline-down',
  ...
  const sideCursors: Record<TimelineBranchSide, number> = {
 ...
 * 判断时间轴一级主题挂在轴线上方还是下方。
 */
export function rootSubtopicTimelineSide(
  index: number,
 ...
 */
export function placeTimelineDetails(
  parent: LayoutTopic,
  branchSide: TimelineBranchSide,
  collapsedIds: ReadonlySet<string>,
  branchExpansion: BranchExpansion = 'side'
): void {
  ...
  const axisBandHalfHeight =
    parentBox.timelineAxisBandHalfHeight !== undefined &&
    Number.isFinite(parentBox.timelineAxisBandHalfHeight)
 ...
 */
export function placeTimelineHangingDetails(
  parent: LayoutTopic,
  branchSide: TimelineBranchSide,
  collapsedIds: ReadonlySet<string>,
 ...
 */
export function placeTimelineNaturalDetails(
  parent: LayoutTopic,
  branchSide: TimelineBranchSide,
  collapsedIds: ReadonlySet<string>,
  branchExpansion: BranchExpansion
 ...
 */
export function timelinePointWidth(
  topic: LayoutTopic,
  collapsedIds: ReadonlySet<string>,
  branchExpansion: BranchExpansion = 'side'
): number {
 ...
 */
export function timelineVisibleSubtreeHorizontalBoundary(
  topic: LayoutTopic,
  collapsedIds: ReadonlySet<string>
): number {
  const box = topic._layout;
 ...
 */
function timelineDetailSubtreeExtent(
  topic: LayoutTopic,
  branchSide: TimelineBranchSide,
  collapsedIds: ReadonlySet<string>,
  branchExpansion: BranchExpansion = 'side'
