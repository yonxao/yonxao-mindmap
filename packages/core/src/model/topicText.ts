...
export interface TopicTextSplit {
  head: string;
  tail: string;
}
 ...
 */
export function wrapTopicText(text: unknown, maxUnits: number): string[] {
 ...
 * 把单个硬换行段拆成视觉宽度合适的多行。
 */
function wrapSingleTopicLine(text: string, maxUnits: number): string[] {
 ...
 */
export function wrapTopicTextByWidth(
  text: unknown,
  maxWidth: number,
  font: TopicFont = {}
): string[] {
 ...
 * 按像素宽度拆分单个硬换行段。
 */
function wrapSingleTopicLineByWidth(text: string, maxWidth: number, font: TopicFont): string[] {
  const normalized = normalizeHorizontalWhitespace(text);
  if (!normalized) return [];
 ...
 */
function normalizeHardLines(text: unknown): string[] {
  return String(text || '')
    .replace(/\r\n/g, '\n')
    .split('\n')
 ...
 * 归一化行内空白，但不把空白当成换行语义。
 */
function normalizeHorizontalWhitespace(text: unknown): string {
  return String(text || '')
    .replace(/[^\S\r\n]+/g, ' ')
    .trim();
}
 ...
 */
export function shouldWrapByWords(text: string): boolean {
  return text.includes(' ') && !CJK_OR_FULLWIDTH_RE.test(text);
}

function pushWrappedLine(lines: string[], line: string): void {
  const normalized = trimLineEnd(line);
  if (normalized) lines.push(normalized);
}

function trimLineStart(text: unknown): string {
  return String(text || '').replace(/^[^\S\r\n]+/, '');
}

function trimLineEnd(text: unknown): string {
  return String(text || '').replace(/[^\S\r\n]+$/, '');
}
 ...
 */
export function normalizeTopicTextForStorage(text: unknown): string {
 ...
 */
export function splitByUnits(text: string, maxUnits: number): TopicTextSplit {
  let units = 0;
  let index = 0;
 ...
 */
export function splitByEstimatedWidth(
  text: string,
  maxWidth: number,
  font: TopicFont = {}
): TopicTextSplit {
 ...
 */
export function visualUnits(text: unknown): number {
  let count = 0;
 ...
 * 用少量宽度换取不会穿出主题框的稳定显示。
 */
export function estimateTopicTextWidth(text: unknown, font: TopicFont = {}): number {
...
function estimateTopicCharWidth(char: string, fontSize: number): number {
