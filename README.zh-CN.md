...
- 主题属性写在主题级别标记行末尾，格式为 `[key=value]`；多行主题的属性仍然放在第一行末尾。
- 当前支持的属性：
  - `color=#3b82f6`：主题颜色。
...
| 中文术语      | English Term           | yxmm 术语 / 配置名                                 | 作用或功能说明                               |
| ------------- | ---------------------- | -------------------------------------------------- | -------------------------------------------- |
| 悬浮工具栏    | floating toolbar       | `basic.toolbar.corner` / `basic.toolbar.placement` | 放置常用操作按钮，可以拖动并吸附到角落。     |
...
| 高度拖拽条    | height resize handle   | `basic.canvasHeight` / `basic.sourceHeight`        | 调整导图幕布或源码模式高度。                 |

### 视觉与交互区域

| 中文术语     | English Term         | yxmm 术语 / 配置名        | 作用或功能说明                                           |
| ------------ | -------------------- | ------------------------- | -------------------------------------------------------- |
| 幕布         | canvas               | `basic.canvasHeight`      | 承载 SVG 导图的可视区域。                                |
| 视口         | viewport             | -                         | 当前 SVG 中正在被查看的坐标范围；平移和缩放会改变视口。  |
| 主题卡片     | topic card           | -                         | 主题的矩形外观，包括背景、边框和圆角。                   |
| 主题色系     | theme                | -                         | 控制整体颜色方案，避免和“主题 / Topic”混淆。             |
| 默认主题颜色 | default topic color  | `theme.defaultTopicColor` | 覆盖主题自动配色；主题属性 `color` 仍然优先。            |
| 字体         | font                 | `font.*`                  | 控制主题文字样式。                                       |
| 连线线型     | connector style      | `layout.connectorStyle`   | 控制连线为曲线、直线或折线；目前仅思维导图组布局可配置。 |
| 平移         | pan                  | -                         | 拖动幕布改变视口位置。                                   |
| 缩放         | zoom                 | -                         | 放大或缩小当前视口。                                     |
| 适配视图     | fit view             | -                         | 自动调整视口，让导图尽量完整显示。                       |
| 重置折叠状态 | reset collapse state | -                         | 清除当前折叠记录，展开所有主题。                         |
| 自动高度     | auto height          | -                         | 根据导图内容自动计算幕布高度。                           |
| 手动高度     | manual height        | `basic.canvasHeight`      | 用户拖拽后保存的幕布高度。                               |
...
Obsidian 的 `设置` -> `第三方插件` -> `yonxao-mindmap` 中提供了插件级别的“全局默认值配置面板”。
...
- `basic.canvasHeight`：幕布高度。拖动幕布底部边缘后会自动写入；双击拖拽条会移除该项并恢复自动高度。
- `basic.sourceHeight`：源码模式高度，和导图模式的 `basic.canvasHeight` 分开保存。
- `basic.toolbar.corner` / `basic.toolbar.placement`：悬浮工具栏吸附位置。`corner` 可选 `top-left`、`top-right`、`bottom-left`、`bottom-right`；`placement` 可选 `inside`、`outside`。拖动工具栏抓手后会自动吸附到最近的位置并写入。
- `basic.viewFit`：打开导图时的视图适配方式，可选 `original`、`fit`。默认 `fit`，会尽量显示完整导图；`original` 按原始大小显示。
- `basic.fitViewNoUpscale`：适配视图是否禁止放大小图，默认开启。开启后适配视图只缩小过大的导图，不放大原本较小的导图。
- `basic.fitViewMaxScale`：关闭 `basic.fitViewNoUpscale` 时的适配视图最大放大倍数，默认 `1.5`。
- `basic.tabIndent`：源码模式中是否启用 `Tab` / `Shift+Tab` 调整主题级别，默认开启。
- `basic.wheelZoom`：是否启用鼠标滚轮缩放，默认关闭。
- `theme.scheme`：主题名称，可选 `default`、`ocean`、`forest`、`sunset`、`mono`、`rainbow`、`pastel-rainbow`、`neon-rainbow`。
- `theme.defaultTopicColor`：默认主题颜色，会覆盖主题自动配色；主题属性 `color` 仍然优先。
- `layout.type`：布局类型，可选 `mindmap-right`、`mindmap-left`、`mindmap-bidirectional`、`mindmap-down`、`mindmap-up`、`mindmap-vertical`、`tree`、`tree-right`、`tree-left`、`org`、`org-right`、`timeline`、`timeline-up`、`timeline-down`、`radial`、`fishbone-left`、`fishbone-right`、`tree-table`、`tree-table-stepped`。
- `layout.connectorStyle`：连线线型，仅思维导图组布局可配置，可选 `curve`、`straight`、`elbow`。`curve` 是曲线，技术上是三次贝塞尔曲线；`straight` 是直线；`elbow` 是正交折线。树形图、组织结构图、时间轴、放射图、鱼骨图、树形表格等非思维导图布局会固定使用折线。
- `layout.branchExpansion`：普通主题的子主题展开方式，可选 `side`、`hanging`。
- `layout.topicMaxWidth.global`：全局主题最大宽度，长标题会按这个宽度换行。
- `layout.topicMaxWidth.level1`、`layout.topicMaxWidth.level2`、`layout.topicMaxWidth.level3`：按主题级别设置最大宽度。
- `font.family`、`font.size`、`font.weight`、`font.lineHeight`：全局默认字体。
- `font.level1`、`font.level2`、`font.level3`：按主题级别设置字体，数字对应 `#`、`##`、`###` 的层级。
...
`主题属性 maxWidth` > `layout.topicMaxWidth.levelN` > `layout.topicMaxWidth.global` > `插件默认值`
...
`主题属性 color` > `theme.defaultTopicColor` > `theme 自动配色`

其中 `rainbow`、`pastel-rainbow`、`neon-rainbow` 会按一级分支自动分配不同颜色。
中心主题会使用主题独立中心色；主题属性 `color` 只改变主题本身，不改变它和父主题之间的连接线颜色。
配置区中的 hex 颜色建议写成带引号的字符串，例如 `defaultColor: '#66ed0c'`；通过配置面板保存时会自动加引号。
...
- 放射图：`radial` 放射图。
- 鱼骨图：`fishbone-left` 左向鱼骨图、`fishbone-right` 右向鱼骨图。
...
- 编辑视图的导图模式中，悬停主题并点击主题上的小编辑按钮，可以编辑内容、颜色、图标和布局。
- 工具栏支持适配视图、放大、缩小和重置折叠状态。
- 工具栏中的配置按钮可以打开可视化配置面板；常用项可用下拉框选择，高级页也可以直接编辑 YAML 配置。
- 拖动工具栏左侧抓手可以把工具栏吸附到最近的角落内侧或外侧，避免遮挡导图内容。
- 拖动幕布可以平移视图。默认情况下鼠标滚轮会继续滚动 Obsidian 页面；在配置面板“基础”页开启“启用鼠标滚轮缩放”后，滚轮会缩放当前导图。
- 拖动幕布底部边缘可以手动调整幕布高度，双击拖拽条可以恢复自动高度。
- Obsidian 自带的“编辑这个块”按钮会继续保留；插件工具栏可吸附到四角内侧或外侧，必要时避开它。
