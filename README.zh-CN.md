...
| 中文术语      | English Term           | yxmm 术语 / 配置名                     | 作用或功能说明                               |
| ------------- | ---------------------- | -------------------------------------- | -------------------------------------------- |
| 悬浮工具栏    | floating toolbar       | `toolbar.corner` / `toolbar.placement` | 放置常用操作按钮，可以拖动并吸附到角落。     |
...
| 连线线型     | connector style      | `connector.style`    | 控制连线为曲线、直线或折线；目前仅思维导图组布局可配置。 |
| 平移         | pan                  | -                    | 拖动幕布改变视口位置。                                   |
| 缩放         | zoom                 | -                    | 放大或缩小当前视口。                                     |
...
- `toolbar.corner` / `toolbar.placement`：悬浮工具栏吸附位置。`corner` 可选 `top-left`、`top-right`、`bottom-left`、`bottom-right`；`placement` 可选 `inside`、`outside`。拖动工具栏抓手后会自动吸附到最近的位置并写入。
...
- `layout`：布局类型，可选 `mindmap-right`、`mindmap-left`、`mindmap-bidirectional`、`mindmap-down`、`mindmap-up`、`mindmap-vertical`、`tree`、`tree-right`、`tree-left`、`org`、`org-right`、`timeline`、`timeline-up`、`timeline-down`、`radial`、`fishbone-left`、`fishbone-right`、`tree-table`、`tree-table-stepped`。
- `connector.style`：连线线型，仅思维导图组布局可配置，可选 `curve`、`straight`、`elbow`。`curve` 是曲线，技术上是三次贝塞尔曲线；`straight` 是直线；`elbow` 是正交折线。树形图、组织结构图、时间轴、放射图、鱼骨图、树形表格等非思维导图布局会固定使用折线。
- `font.family`、`font.size`、`font.weight`、`font.lineHeight`：全局默认字体。
- `font.levels.1`、`font.levels.2`、`font.levels.3`：按主题级别设置字体，数字对应 `#`、`##`、`###` 的层级。
- `topic.defaultColor`：默认主题颜色，会覆盖主题自动配色；主题属性 `color` 仍然优先。
- `topic.maxWidth`：全局主题最大宽度，长标题会按这个宽度换行。
- `topic.levels.1.maxWidth`、`topic.levels.2.maxWidth`、`topic.levels.3.maxWidth`：按主题级别设置最大宽度。
...
`主题属性 maxWidth` > `topic.levels[当前级别].maxWidth` > `topic.maxWidth` > `插件默认值`
