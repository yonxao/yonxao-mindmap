...
- 图片布局阶段不读取真实图片尺寸，只按语法尺寸提示或默认比例估算；显式像素宽度如 `|320`、`|400x300` 会参与主题宽度估算，必要时撑开图片主题但仍受主题最大宽度硬上限约束，默认宽度为 `min(220px, 主题可用文本宽度)`，未指定高度时按 0.62 的兜底比例估算；`|50%` 按当前主题可用文本宽度计算，`|original` 主要用于双击预览时切换原始尺寸。
...
- 阅读视图/窗口全屏的 body 级覆盖层必须先挂到 `document.body`，再把 `hostEl` 移入覆盖层，避免 `hostEl` 在 Obsidian 预览渲染和图片异步加载回调之间短暂脱离文档树；全屏请求失败时也要同时移除 `is-fullscreen` 并恢复工具栏到 body。
