...
高级结构自动引用主题时使用 `t-` 加三位数字的稳定 ID，例如 `t-123`；结构自身分别使用 `r-123`、`s-123`、`b-123`。已有 ID 不主动迁移，避免破坏已保存的结构引用。关联默认使用正向曲线，裁剪保存会省略对应默认属性；关联线需要保持轻微透明，并优先选择不穿过可见主题卡片的局部绕行路径。选中关联后，两端主题显示 8 个固定锚点，拖动端点后分别保存为 `fromAnchor` / `toAnchor`；未保存锚点时继续使用自动路由。关联始终位于概要上层，曲线另显示两个可拖动控制点；高级结构焦点和主题焦点互斥。
...
外框标题与上方主题冲突时，`reserveBoundaryLabelSpace()` 通过 `structureBounds.js` 为当前边界及其后续布局整体插入垂直空间。不能只移动与外框水平重叠的同列主题，否则多个外框累计避让后会破坏树形图的父子对齐。
...
- 水印条通过专用内容层标记裁剪主题、连线和高级结构，为顶部或底部水印条预留区域；不能给编辑控件或任意 SVG 兄弟元素写入裁剪规则。
- 平铺水印限制单次元素数量；偏移、尺寸和间距必须先经过配置归一化，图片地址在单次渲染和导出中复用解析结果，避免极端配置或重复资源拖慢主功能。
...
- 配置面板的颜色字段统一复用 `createColorTextField`，按“值输入、原生色盘、预设色块”排列；透明预设只由允许透明的字段显式开启。
- 保存代码块配置时，如果某项值和当前全局配置一致，应从代码块配置中删除该项，让配置区保持精简。
- 全局配置面板和代码块配置面板的默认值、下拉框回填、禁用态和说明文案应保持一致。
- 移动端配置面板必须避开系统状态栏、手势栏等安全区域；高度、宽度和拖拽边界都要读取共享 safe area 变量，不能只按 `100vh` / `100vw` 贴边计算。
...
- 移动端普通阅读/编辑页面中，单指纵向滑动导图区应优先滚动 Obsidian 页面，不应默认平移导图视口。
...
- 移动端只有底部中间的高度抓手能启动触摸拖拽，抓手默认不常显，开始拖动后才显示反馈；底部其它区域应继续让页面纵向滚动，避免误改 `display.canvasHeight` / `display.sourceHeight`；手动高度也可通过配置面板或非触摸指针调整。
...
- 工具栏吸附点和拖拽边界必须按视口 safe area 裁剪；移动端“上方外侧”不能被吸到系统状态栏下方不可点击区域。
- 拖动工具栏不应触发视图从源码模式切回导图模式。
- 退出全屏时必须隐藏当前工具栏；显示某个 renderer 的工具栏前要隐藏页面上其它 `yxmm` renderer 的可见工具栏，避免移动端多代码块页面出现残留工具栏。
- 工具栏透明度有过调整，避免遮挡导图内容。
- 工具栏不要和 Obsidian 自带"编辑这个块"按钮冲突，必要时偏左上放置。
- 导图空白处右键菜单应覆盖工具栏中的查看类入口，包括源码/导图切换、配置、适配视图、原始大小、窗口全屏、物理全屏、放大、缩小和重置折叠状态；移动端用户不应只能依赖悬浮工具栏完成这些操作。
...
- 移动端全屏覆盖层必须避开系统安全区域；顶部状态栏下方不可点击，工具栏、关闭按钮和导图可交互内容都不能放进这一区域。
- 物理全屏和窗口全屏进入、退出后都要重新刷新视图、工具栏位置和 SVG DOM 焦点；浏览器或 Obsidian 可能把焦点落到 `body`，如果不补焦点，`Alt/Option` 全屏快捷键可能无法连续退出或再次进入。
- 物理全屏和窗口全屏下的适配视图必须按当前幕布宽高比补齐 `viewBox` 的短边，让内容水平和垂直都居中；不能依赖 SVG 全局 `preserveAspectRatio`，因为它为了 PDF 跨页导出使用顶部对齐。
- 移动端全屏中，触摸拖动用于平移导图视口，并阻断 Obsidian 自身下拉命令面板、返回等页面级手势。
...
- PNG 导出使用 SVG 克隆路径：复用包含高级结构和外框标题占位的 `renderedMapBounds`，内联计算后的 SVG 样式和图片数据，并移除主题/结构焦点、透明命中路径和关联控制点。导出根 SVG 必须固化当前 `--background-primary`，否则导出样式表会覆盖概要背景和文字描边的展示属性，并在深色模式错误回退为白色。公式 foreignObject 替换为 MathJax SVG、当前屏幕中真实公式节点的截图，或轻量 SVG 兜底。兜底公式也必须按页面中真实 MathJax/HTML 区块的矩形定位和缩放，避免复制图片里的公式字号、基线、分式高度明显偏离预览。不要优先使用 Electron `capturePage` 分片截图导出整张图；长图分片容易把视口内容重复拼接，导致导出图出现重复段。
...
布局结果由 core 产生；高级结构 ID、主题稳定 ID 分配和主题变化后的失效结构清理也由 core 统一；主题控件语义点、高级结构路径/边界和水印布局由 svg-renderer 产生。插件层只保留宿主设置存储、UI 文案和选项元数据、
