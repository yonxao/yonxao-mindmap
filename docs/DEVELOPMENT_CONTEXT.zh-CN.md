...
内容局部样式属于主题内容的一部分，不属于主题属性，也不进入配置系统。导图编辑保存时应保留这些标记文本；布局测量时使用去掉标记后的显示文本；SVG 渲染时再按片段生成不同样式的 `tspan`。
...
普通段落文本支持 `auto` / `left` / `center` / `right` 四种对齐方式。`auto` 是默认值，会采用方向感知对齐：一级主题居中；右向分支左对齐；左向分支右对齐，让文字重心贴近对应连接侧。显式 `left`、`center`、`right` 会覆盖方向判断；列表、代码块等块级内容保持左对齐，避免破坏阅读和缩进结构。
...
- `align`：普通段落文本对齐方式，可选 `auto`、`left`、`center`、`right`；`auto` 根据布局方向自动判断。
...
- PNG 导出使用 SVG 克隆路径：重新计算 bounds、内联计算后的 SVG 样式、内联图片数据，并把公式 foreignObject 替换为 MathJax SVG、当前屏幕中真实公式节点的截图，或轻量 SVG 兜底。兜底公式也必须按页面中真实 MathJax/HTML 区块的矩形定位和缩放，避免复制图片里的公式字号、基线、分式高度明显偏离预览。不要优先使用 Electron `capturePage` 分片截图导出整张图；长图分片容易把视口内容重复拼接，导致导出图出现重复段。
...
源码模式快捷键只在按键事件来自当前源码视图时生效：

| 操作     | Windows | Mac        | 行为说明                                                       |
| -------- | ------- | ---------- | -------------------------------------------------------------- |
| 保存源码 | `Alt+S` | `Option+S` | 写回当前 Markdown 代码块，保持源码模式并更新底部保存状态提示。 |
