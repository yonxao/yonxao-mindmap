...
主题快捷键只应在导图 SVG 获得焦点、且当前有主题焦点时生效。`Tab`、`Enter`、`Space`、`Delete` 等按键在 Obsidian 或浏览器中有原生语义，确认命中主题快捷键后必须 `preventDefault()` 和 `stopPropagation()`。

当前已确认快捷键：

| 操作               | Windows        | Mac                        | 行为说明                                     |
| ------------------ | -------------- | -------------------------- | -------------------------------------------- |
| 插入子主题         | `Tab`          | `Tab`                      | 为当前焦点主题新增子主题。                   |
| 插入弟主题         | `Enter`        | `Return`                   | 在当前焦点主题后新增同级主题。               |
| 插入兄主题         | `Shift+Enter`  | `Shift+Return`             | 在当前焦点主题前新增同级主题。               |
| 删除主题           | `Delete`       | `Delete` / `Cmd+Backspace` | 删除当前焦点主题，确认后回父主题。           |
| 打开主题编辑面板   | <code>`</code> | <code>`</code>             | 只用反引号；不要再绑定 F2。                  |
| 快速内联编辑       | `Space`        | `Space`                    | 打开当前主题的内联编辑框。                   |
| 内联编辑中换行     | `Shift+Enter`  | `Shift+Return`             | 在主题文本内插入换行。                       |
| 内联编辑中提交     | `Enter`        | `Return`                   | 保存文本并回到当前主题焦点。                 |
| 内联编辑中取消     | `Esc`          | `Esc`                      | 放弃修改并回到当前主题焦点。                 |
| 主题编辑面板保存   | `Ctrl+S`       | `Cmd+S`                    | 保存整个主题编辑面板。                       |
| 长文本编辑浮层保存 | `Ctrl+S`       | `Cmd+S`                    | 只触发长文本浮层“保存”，不保存整个主题面板。 |
...
- 主题快捷键不应在源码模式、阅读视图编辑禁用场景、输入法组合输入中触发。
